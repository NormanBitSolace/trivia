...
extension UIView {
    ...
    func constrain(to target: UIView) {
        translatesAutoresizingMaskIntoConstraints = false

        if #available(iOS 11.0, *) {
            target.addConstraints([
                target.leadingAnchor.constraint(equalTo: safeAreaLayoutGuide.leadingAnchor),
                target.topAnchor.constraint(equalTo: safeAreaLayoutGuide.topAnchor),
                target.trailingAnchor.constraint(equalTo: safeAreaLayoutGuide.trailingAnchor),
